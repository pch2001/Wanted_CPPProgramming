...
public:
	...
	// 배열의 첫 위치를 반환하는 함수
	//Iterator begin() -> 구지 iterator로 안 감싸도 된다.
	T* begin()
	{
		//return Iterator(data); -> 구지 iterator로 안 감싸도 된다.
		return data;
	}

	// 배열에 저장된 마지막 요소의 다음 위치를 반환하는 함수
	//Iterator end() -> 구지 iterator로 안 감싸도 된다.
	T* end()
	{
		//return Iterator(data + size); -> 구지 iterator로 안 감싸도 된다.
		return data + size;
	}
