...
Node::Node(const Bounds& bouds, int depth) : bounds(bouds), depth(depth)
{
...
bool Node::Subdivide()
{
	...
	if (!IsDivided())
	{
		...
		int x = bounds.X();
		int y = bounds.Y();
		int halfWidgth = bounds.Width() / 2;
		int halfHeight = bounds.Height() / 2;

		// 각 4분면의 자식 개겣 생성
		topLeft = new Node(Bounds(x, y, halfWidgth, halfHeight), depth+1);

		topRight = new Node(Bounds(x + halfWidgth, y, halfWidgth, halfWidgth), depth + 1);

		bottomLeft = new Node(Bounds(x, y + halfWidgth, halfWidgth, halfWidgth), depth + 1);

		bottomRight = new Node(Bounds(x + halfWidgth, y + halfHeight, halfWidgth, halfWidgth), depth + 1);
...
bool Node::IsDivided()
{
	// 분할인 됐다면 모든 자식 노드가 null이 아니기 때문에
...
std::vector<NodeIndex> Node::GetQuads(const Bounds& bounds)
{
	...
	// 아래쪽 영역이랑 겹치는지 확인
	bool bottom = bounds.MaxX() >= centerY && bounds.MaxY() >= this->bounds.MaxY();
